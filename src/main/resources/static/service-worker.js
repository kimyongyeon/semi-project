...
// 응답 및 네트워크를 통한 캐시 업데이트
// self.addEventListener('fetch', (event) => {
//    // event.respondWith(
//    //     caches.open(CACHE_NAME).then((cache)=>{
//    //         return cache.match(event.request).then((cacheResponse) => {
//    //             const fetchPromise = fetch(event.request).then((networkRequest) => {
//    //                 if (!/^https?:$/i.test(new URL(event.request.url).protocol)) return networkRequest;
//    //                 cache.put(event.request, networkRequest.clone());
//    //                 return networkRequest;
//    //             });
//    //
//    //             // 캐시가 존재하면 캐시 응답, 존재하지 않으면 네트워크 응답 반환
...
self.addEventListener('fetch', (event) => {
    ...
    //         // return fetch(getContextPath() + '/img/dog.jpg');
    //     })
    // );
